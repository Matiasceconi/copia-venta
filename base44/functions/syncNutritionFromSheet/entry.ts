...
Deno.serve(async (req) => {
  try {
    ...
    const configRecords = await base44.asServiceRole.entities.IntegrationConfig.filter({
      organization_id: activeOrgId, key: 'nutrition_source_file_id',
    }, '-updated_at', 1);
    const sourceFileId = (configRecords[0]?.value || '').trim();
    ...
    if (!sourceFileId) {
      return Response.json({
        success: false,
        disabled: true,
        message: 'La sincronización nutricional está desactivada. Un administrador debe configurar el source_file_id desde la configuración de integraciones.',
      });
    }
